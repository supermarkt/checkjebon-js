...
const { getOptimalShoppingPlan, getSupermarkets } = require('../checkjebon');

describe('getOptimalShoppingPlan', function () {
  ...
  it('should return an object with totalCost and supermarkets array', async () => {
    ...
    const result = await getOptimalShoppingPlan(testProducts, 2, selectedStores);
  ...
  it('should respect the maxSupermarketVisitCount limit', async () => {
    ...
    const maxVisits = 2;
    const result = await getOptimalShoppingPlan(testProducts, maxVisits, selectedStores);
  ...
  it('should only return selected supermarkets', async () => {
    ...
    const result = await getOptimalShoppingPlan(testProducts, 3, selectedStores);
  ...
  it('should throw error if no matching supermarkets found', async () => {
    ...
    try {
      await getOptimalShoppingPlan(testProducts, 2, selectedStores);
  ...
  it('should work with maxSupermarketVisitCount of 1', async () => {
    const selectedStores = ["ah", "dirk", "jumbo"];
    const result = await getOptimalShoppingPlan(testProducts, 1, selectedStores);
